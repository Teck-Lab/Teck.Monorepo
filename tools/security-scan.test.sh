...
#
# 1. changed-mode SAST target selection drops tracked symlinks (mode 120000)
...
#    staged index.
# 3. --pre-push mode validates the git hook's ref update stream, scopes Gitleaks
#    to the introduced commits, fails closed on empty/invalid input, and fixes
#    the base to origin/main.
set -euo pipefail
