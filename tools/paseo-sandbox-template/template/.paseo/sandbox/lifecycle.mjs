...
const commandScripts =
  process.env.NODE_ENV === "test"
    ? {
        sbx: process.env.PASEO_SBX_SCRIPT || process.env.TECK_SBX_SCRIPT,
        git: process.env.PASEO_GIT_SCRIPT || process.env.TECK_GIT_SCRIPT,
      }
    : {};
...
function runtimeDescriptor(workspacePath, gitContext, config = loadWorkspaceConfig(workspacePath)) {
  const omniRouteBaseUrl =
    process.env.PASEO_OMNIROUTE_BASE_URL || process.env.TECK_OMNIROUTE_BASE_URL || config.omniRouteBaseUrl || defaultOmniRouteBaseUrl;
...
function configuredApiKey() {
  ...
  const candidates = [
    process.env.PASEO_OMNIROUTE_ENV_FILE?.trim(),
    process.env.TECK_OMNIROUTE_ENV_FILE?.trim(),
    join(homedir(), ".config", "paseo", "omniroute.env"),
  ...
  throw new Error(
    "OmniRoute key not found; configure ~/.config/paseo/omniroute.env or PASEO_OMNIROUTE_ENV_FILE",
  );
}

function configuredSigningKey() {
  const path =
    process.env.PASEO_GPG_SIGNING_KEY_FILE?.trim() ||
    process.env.TECK_GPG_SIGNING_KEY_FILE?.trim() ||
    [
      join(homedir(), ".config", "paseo", "sandbox-signing-key.asc"),
...
function acquireLock(name) {
  const root = join(tmpdir(), "paseo-sandboxes");
  const directory = join(root, `${name}.lock`);
  const pidPath = join(directory, "pid");
  mkdirSync(root, { recursive: true });
  const deadline = Date.now() + Number(process.env.PASEO_SANDBOX_LOCK_TIMEOUT_MS || process.env.TECK_PASEO_LOCK_TIMEOUT_MS || 300000);
  while (true) {
    try {
      mkdirSync(directory);
...
export function validateWorkspace(workspacePath) {
  const requiredFiles = [
    join(".paseo", "sandbox", "config.json"),
    join(".paseo", "sandbox", "kit", "spec.yaml"),
    ...requiredOmpFiles.map((file) => join(".omp", file)),
  ];
  const missing = requiredFiles.filter((file) => !existsSync(join(workspacePath, file)));
  if (missing.length > 0) {
    throw new Error(`Workspace is missing sandbox prerequisites: ${missing.join(", ")}`);
  }
  loadWorkspaceConfig(workspacePath);
  return resolveGitContext(workspacePath);
}
...
export function shouldRemoveFailedSandbox(created, env = process.env) {
  const keepFailed = env.PASEO_SANDBOX_KEEP_FAILED || env.TECK_PASEO_KEEP_FAILED;
  return created && keepFailed !== "1";
}
...
function ensureSandbox(workspacePath, options = {}) {
  ...
  try {
    ...
    if (!sandboxExists(name)) {
      const image = process.env.PASEO_SANDBOX_IMAGE || process.env.TECK_PASEO_SANDBOX_IMAGE || config.image || defaultImage;
      const cpus = String(process.env.PASEO_SANDBOX_CPUS || process.env.TECK_PASEO_SANDBOX_CPUS || config.cpus || 4);
      const memory = String(process.env.PASEO_SANDBOX_MEMORY || process.env.TECK_PASEO_SANDBOX_MEMORY || config.memory || "4g");
    ...
    if (shouldRemoveFailedSandbox(created)) {
      try {
        run("sbx", ["rm", "--force", name], { capture: true });
      } catch (cleanupError) {
