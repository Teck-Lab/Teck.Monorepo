...
public static class StockReservedV2Handler
{
    ...
    public static Task Handle(StockReservedV2IntegrationEvent evt, IGenericWriteRepository<Order, Guid> orders, ITenantInfo tenant, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct) =>
        Apply(evt.OrderId ?? evt.SourceId, evt.TenantId, evt.IdempotencyKey, evt.SourceCorrelationId, evt.Lines.Any(line => line.BackorderedQuantity > 0), orders, tenant, unitOfWork, bus, ct);
    ...
    /// <param name="ct">The cancellation token.</param>
    /// <returns>A task that completes after reconciliation.</returns>
    public static Task Handle(StockReservedIntegrationEvent evt, IGenericWriteRepository<Order, Guid> orders, ITenantInfo tenant, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct) =>
        Apply(evt.SourceId, evt.TenantId, $"legacy-stock-reserved:{evt.ReservationId:N}", evt.Id.ToString("N"), hasOutstandingBackorder: false, orders, tenant, unitOfWork, bus, ct);

    private static async Task Apply(Guid orderId, string payloadTenantId, string key, string correlation, bool hasOutstandingBackorder, IGenericWriteRepository<Order, Guid> orders, ITenantInfo tenant, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct)
    {
