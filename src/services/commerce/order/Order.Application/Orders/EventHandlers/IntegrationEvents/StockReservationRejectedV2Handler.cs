...
public static class StockReservationRejectedV2Handler
{
    ...
    public static Task Handle(StockReservationRejectedV2IntegrationEvent evt, IGenericWriteRepository<Order, Guid> orders, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct) =>
        Apply(evt.OrderId ?? evt.SourceId, evt.TenantId, evt.IdempotencyKey, evt.SourceCorrelationId, orders, unitOfWork, bus, ct);
    ...
    /// <param name="ct">The cancellation token.</param>
    /// <returns>A task that completes after reconciliation.</returns>
    public static Task Handle(StockReservationRejectedIntegrationEvent evt, IGenericWriteRepository<Order, Guid> orders, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct) =>
        Apply(evt.SourceId, evt.TenantId, $"legacy-stock-rejected:{evt.ReservationId:N}", evt.Id.ToString("N"), orders, unitOfWork, bus, ct);

    private static async Task Apply(Guid orderId, string tenantId, string key, string correlation, IGenericWriteRepository<Order, Guid> orders, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct)
    {
