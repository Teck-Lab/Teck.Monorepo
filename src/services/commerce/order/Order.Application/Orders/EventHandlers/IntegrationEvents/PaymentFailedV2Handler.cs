...
public static class PaymentFailedV2Handler
{
    ...
    public static Task Handle(PaymentFailedV2IntegrationEvent evt, IGenericWriteRepository<Order, Guid> orders, ITenantInfo tenant, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct) =>
        Apply(evt.OrderId, evt.TenantId, evt.DeclineCategory, evt.ActionText, evt.RequestId, evt.SourceCorrelationId, orders, tenant, unitOfWork, bus, ct);
    ...
    /// <param name="ct">The cancellation token.</param>
    /// <returns>A task that completes after reconciliation.</returns>
    public static Task Handle(PaymentFailedIntegrationEvent evt, IGenericWriteRepository<Order, Guid> orders, ITenantInfo tenant, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct) =>
        Apply(evt.OrderId, evt.TenantId, "generic-decline", "Please provide another payment method.", $"legacy-payment-failed:{evt.PaymentId:N}", evt.Id.ToString("N"), orders, tenant, unitOfWork, bus, ct);

    private static async Task Apply(Guid orderId, string payloadTenantId, string category, string actionText, string key, string correlation, IGenericWriteRepository<Order, Guid> orders, ITenantInfo tenant, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct)
    {
