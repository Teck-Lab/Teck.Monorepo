...
public static class BackorderReadyHandler
{
    ...
    public static async Task Handle(BackorderReadyIntegrationEvent evt, IGenericWriteRepository<Order, Guid> orders, IUnitOfWork unitOfWork, IMessageBus bus, CancellationToken ct)
    {
        ...
        if (order is null || !string.Equals(order.TenantId, evt.TenantId, StringComparison.Ordinal) || !order.ApplyBackorderReady(evt.IdempotencyKey, evt.SourceCorrelationId))
        {
            return;
        }
