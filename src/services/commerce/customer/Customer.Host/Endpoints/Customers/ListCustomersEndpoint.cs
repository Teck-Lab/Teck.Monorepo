...
public sealed class ListCustomersEndpoint(IMessageBus bus)
    : AuthenticatedEndpoint<ListCustomersRequest, IReadOnlyList<CustomerDto>>
{
    /// <inheritdoc/>
    protected override EndpointPermission Permission => new("customer", "read", "public");

    /// <inheritdoc/>
    public override async Task HandleAsync(ListCustomersRequest request, CancellationToken ct)
    {
