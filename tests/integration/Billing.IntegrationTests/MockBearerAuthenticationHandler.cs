...
/// validation without modifying Billing.Host production code.
/// This handler lives EXCLUSIVELY in the test assembly and is injected into the billing
/// host via <c>WebApplicationFactory.ConfigureTestServices</c> — it is never compiled
/// into the production billing binary.
...
internal sealed class MockBearerAuthenticationHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        var claims = new[]
        {
            ...
            // Billing handlers resolve the current tenant via ITenantInfo (Finbuckle), not from
            // claims directly, so no customer_id claim is required here. "sub" is included anyway
            // for parity with a real Keycloak access token shape.
