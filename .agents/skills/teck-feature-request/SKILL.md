...
Read [references/paseo-discovery.md](references/paseo-discovery.md) completely
...
2. Route automatically; do not ask the user which workflow or skill to use:
   ...
   - When facts or a prototype require delegation, use the visible Paseo
...
- create executable sub-issues, blocker edges, or engineering agents;
- create a Git branch, commit, pull request, or product-code change, or create
  any worktree other than the one disposable Paseo prototype exception below;
...
only; never use it to transfer an active Paseo coordinator or worker.

The only permitted branch/worktree exception is a Paseo-managed disposable
