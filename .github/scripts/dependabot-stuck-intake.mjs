...
const ignoredChecks = new Set([
  ...
  // Retired after its checks were consolidated into CI. Historical startup
  // failures remain attached to existing PR heads and must not keep repairs open.
  "PR Validation",
]);
...
export function checkState(checkRuns, statuses, now = Date.now(), graceMinutes = 20) {
  const relevantCheckRuns = checkRuns.filter((check) => !ignoredChecks.has(check.name));
  if (relevantCheckRuns.length === 0 && statuses.length === 0)
    return { state: "pending", failures: [] };
  const pendingChecks = relevantCheckRuns.filter((check) => check.status !== "completed");
  ...
  const newestFailure = Math.max(
    ...failures.map((failure) => Date.parse(failure.completedAt ?? 0)).filter(Number.isFinite),
  );
...
async function upsertStuckIssue(client, owner, repo, pull, failures, existing) {
  ...
  const activeLifecycle = labels.some(
    (label) => label.startsWith("agent:") && label !== "agent:completed",
  );
