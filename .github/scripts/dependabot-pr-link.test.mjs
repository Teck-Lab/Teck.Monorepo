...
import { dependabotIssueMetadata, linkedPullRequestBody, matchingIssues, parseNames } from "./dependabot-pr-link.mjs";
...
test("parses Dependabot issue metadata and dependency lists", () => {
  assert.deepEqual(dependabotIssueMetadata(issues[0].body), { advisory: "ghsa-abcd-1234-5678", package: "foo" });
  assert.deepEqual([...parseNames("foo, @scope/bar")], ["foo", "@scope/bar"]);
});

test("matches an advisory and the dependencies changed by the PR", () => {
  assert.deepEqual(matchingIssues(issues, "GHSA-abcd-1234-5678", "foo").map((issue) => issue.number), [41]);
  assert.deepEqual(matchingIssues(issues, "GHSA-abcd-1234-5678", "foo,bar").map((issue) => issue.number), [41, 42]);
});
