...
#   <cwd>\.paseo\scripts\omp-wrapper.ps1
# and forwards all arguments to it.

$cwd = [IO.Path]::GetFullPath((Get-Location).Path)
$wrapper = Join-Path $cwd '.paseo\scripts\omp-wrapper.ps1'

if (-not (Test-Path -LiteralPath $wrapper -PathType Leaf)) {
    throw "Could not find repo-local OMP wrapper: $wrapper`n`nCopy the '.paseo' folder from the Paseo sandbox template into this repository and commit it."
}
