...
The recipe creates one Docker Sandbox per `ORCA_PROJECT_ID`, clones the Teck project once onto the VM disk, exposes a private `sshd` on a deterministic loopback port, and reuses that sandbox for every Orca worktree in the project. Its base image includes OMP, Bun, and the repository-pinned .NET SDK. It adds Teck OMP configuration, attaches the pinned official GitHub MCP server through Docker Sandboxes' host-managed MCP gateway, configures OmniRoute proxy-managed research, installs the dedicated sandbox GPG key, and verifies the Orca runtime.
